...
public class BOL01JpegJson {
	...
	public void jsonGenerator() throws IOException {
		...
		dataObj.put("bolFileId", jsonconfig.getProperty(Field.BOL_FILE_ID_ONE));
		dataObj.put("bolFileType", jsonconfig.getProperty(Field.BOL_FILE_TYPE_ONE));

		try {
			bolfile = new String((Files.readAllBytes(Paths.get("src/main/resources/demo/BOL_01_jpeg.txt"))));
